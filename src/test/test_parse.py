...
@pytest.mark.parametrize("line,des_task,des_done", [
    ("Task one:: update","Task one",False),
    ("Task one:: task two:: update", task_join(("Task one","task two")), False),
    ("Task one:: task two:: task three:: update (DONE)",  task_join(("Task one","task two","task three")), True)
])
...
@pytest.mark.parametrize("line,des_alias,des_urls,des_posfix,des_order", [
    ("[Alias one] Task one::", {'Alias one':"Task one"},{},{},{}),
    ("[Alias two] Task one:: task two::", {'Alias two': task_join(("Task one","task two"))}, {}, {},{}),
    ("[Alias two] Task one:: task two:: ::POSFIX:blah blah:", {'Alias two': task_join(("Task one","task two"))}, {}, {task_join(('Task one','task two')): 'blah blah'},{}),
    ("[Alias one] Task one:: http://test.com", {'Alias one': "Task one"}, {'Task one':"http://test.com"},{},{}),
    ("[Alias one] Task one:: ::POSFIX:my posfix here:",{'Alias one': "Task one"},{},{'Task one': 'my posfix here'},{}),
    ("[Alias one] Task one:: http://test.com ::POSFIX:my posfix here: ::ORDER:z:",{'Alias one': "Task one"},{'Task one':"http://test.com"},{'Task one': 'my posfix here'},{'Task one': 'z'}),
])
...
@pytest.mark.parametrize("file_content,des_date",[
...
@pytest.mark.parametrize("file_content,des_task, des_update",[
    ("# 2001-01-01\n[T1] task1::\nT1:: update", "task1","update"),
    ("# 2001-01-01\n[T1] task1:: ::POSFIX:my posfix!:\nT1:: update", "task1","update my posfix!"),
    ("# 2001-01-01\n[T1] task1:: task2::\nT1:: task 3:: update", task_join(("task1","task2","task 3")),"update"),
])
...
@pytest.mark.parametrize("lines,des_open_tasks", [
